...
def estimate_flops_with_compression(
    num_params: int,
    seq_len: int,
    ...
    per_token_flops = num_layers * (
        ...
        compressed_seq_len * (num_heads * head_dim) +
...
def evaluate_kvpress(
    ...
    press = SnapKVPress(compression_ratio=compression_ratio, window_size=window_size)
    ...
    for idx in tqdm(range(min(num_samples, len(dataset))), desc=f"Evaluating {dataset_name} with KVPress"):
        sample = dataset[idx]
    ...
    flops = estimate_flops_with_compression(
        num_params=num_params,
        seq_len=int(avg_input_len),
        compressed_seq_len=int(avg_compressed_len),
...
def main():
    parser = argparse.ArgumentParser(description="KVPress evaluation for Pythia-70m")
    ...
    parser.add_argument("--compression_ratio", type=float, default=0.5, help="KV cache compression ratio")
    parser.add_argument("--window_size", type=int, default=64, help="Window size for SnapKVPress")
    parser.add_argument("--device", type=str, default="cpu", help="Device to use")
    parser.add_argument("--output", type=str, default=None, help="Output JSON file path")

    args = parser.parse_args()

    results = evaluate_kvpress(
    ...
    print("\n" + "=" * 50)
    print("KVPRESS EVALUATION RESULTS")
    print("=" * 50)
    print(f"Model: {results['model']}")
    print(f"Dataset: {results['dataset']}")
    print(f"Press: {results['press_class']}")
    ...
    print(f"  Actual Compression: {results['aggregate']['avg_compression_ratio']:.2%}")
    print(f"\nTiming:")
